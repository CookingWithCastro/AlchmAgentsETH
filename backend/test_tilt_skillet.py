    ...
    return schemas.TiltSkilletPlanResponse(
        ...
        stages=[
            schemas.TiltSkilletStage(
                step_number=1,
                name="Sear the base",
                instruction="Sear the beef in batches until deeply browned.",
                add_to_skillet=[
                    schemas.TiltSkilletStageIngredient(ingredient="beef", quantity="4", unit="cup"),
                    schemas.TiltSkilletStageIngredient(ingredient="olive oil", quantity="0.25", unit="cup"),
                ],
                ...
                circuit_role="source",
                reaction_note="High current — fast, lively browning drives the whole circuit.",
                sensory_cues=["deep brown crust", "audible sizzle"],
            ),
            schemas.TiltSkilletStage(
                step_number=2,
                name="Braise the bulk",
                instruction="Level the pan, add stock and aromatics, and braise low and slow.",
                add_to_skillet=[
                    schemas.TiltSkilletStageIngredient(ingredient="stock", quantity="6", unit="cup"),
                    schemas.TiltSkilletStageIngredient(ingredient="carrot", quantity="3", unit="cup"),
                ],
                ...
                circuit_role="load",
                reaction_note="The load: the bulk absorbs the energy banked by the source.",
                sensory_cues=["fork tender", "glossy reduced liquid"],
            ),
        ],
        ...
        finishing_and_serving=schemas.FinishingAndServing(
            garnish_and_plating="Scatter parsley and lemon zest.",
            doneness_cues="Fork slides in with no resistance.",
...
def test_tilt_skillet_plan_route_returns_valid_plan(monkeypatch):
    ...
    response = client.post(
        ...
        json={
            ...
            "cuisine": "French",
            "stages": [
                {"name": "Sear", "ingredients": [{"name": "beef", "quantity": 4, "unit": "cup"}]},
                {"name": "Braise", "ingredients": [{"name": "stock", "quantity": 6, "unit": "cup"}]},
            ],
            "circuitContext": {"series": {"totalVoltage": -0.8}},
        },
    )
    ...
    # default tier for the planner is "primary" (a good model for a premium surface)
    assert captured["tier"] == "primary"
    assert captured["stages"] == 2


def test_tilt_skillet_plan_route_normalizes_aliases(monkeypatch):
    captured = {}

    async def fake_generate(request, tier, anthropic_model):
        captured["batchServings"] = request.batchServings
        captured["dietPreference"] = request.dietPreference
        return _canned_plan()
    ...
    response = client.post(
        "/api/tilt-skillet-plan",
        json={"prompt": "vegan batch", "servings": 40, "diet": "vegan"},
    )

    assert response.status_code == 200, response.text
    assert captured["batchServings"] == 40
    assert captured["dietPreference"] == "vegan"
