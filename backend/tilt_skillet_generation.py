...
def _set_cached_plan(cache_key: str, plan: schemas.TiltSkilletPlanResponse) -> None:
    ...
    with _CACHE_LOCK:
        _CACHE[cache_key] = (
            time.time() + TILT_SKILLET_CACHE_TTL_SECONDS,
            plan.model_copy(deep=True),
        )


def _compact_plan_schema() -> str:
    schema = schemas.TiltSkilletPlanResponse.model_json_schema()
...
def _format_context(request: schemas.TiltSkilletRequest) -> str:
    context = {
        ...
        "circuitReadings": request.circuitContext,
        "mcpIngredientProfile": request.mcpIngredientProfile,
    }
...
- One stage per element of the circuit. Set each stage's circuit_role from the precomputed circuitReadings (per-stage role); the series order should flow source → ... → load.
- In reaction_note, explain the stage through its circuit reading (voltage/current/resistance/power). Do not invent different physics — honor the provided numbers.
- circuit_summary must reflect the provided series readings (total_voltage, total_current, total_resistance, total_power, impedance, kalchm, monica).
...
async def generate_tilt_skillet_plan(
    ...
    for attempt in range(2):
        user_message = _build_plan_prompt(
            request,
    ...
    raise HTTPException(
        status_code=502,
        detail={
            "error": "Tilt Skillet model returned malformed output after retry",
            "message": last_error,
        },
    )
